...
#from api_server.models.alerts import Al
import aiohttp
...
async def update_cart_status(cart_status: CartStatus, api_server: str, headers: dict[str, str]) -> CartStatus:
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        data = {
            ...
            "type": "smart_cart",
            "data": json.loads(cart_status.json()) #horrible hack cause we are still on pydantic 1.x. In pydantic 2.x we can use dict(rountrip=true)
        }
